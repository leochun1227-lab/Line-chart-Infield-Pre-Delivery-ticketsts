...
function renderChart(series) {
  ...
  claimsChart = new Chart(chartCanvas, {
    ...
    options: {
      ...
      plugins: {
        legend: { position: 'right', align: 'start', labels: { usePointStyle: true, boxWidth: 8, padding: 14 } },
...
function syncControls(data) {
  ...
  const defaultYear = years.map(String).includes(currentYear) ? currentYear : latestYear;

  yearSelect.innerHTML = years
    .map((year) => `<option value="${year}">${year}</option>`)
    .join('');
  yearSelect.value = years.map(String).includes(previousYear) ? previousYear : defaultYear;
...
function getSelectedYearRange(data) {
  const selectedYear = yearSelect.value || getCurrentMonthKey().slice(0, 4);
  const currentYear = getCurrentMonthKey().slice(0, 4);
...
function renderSelectedRange() {
  ...
  const prefix = customRangeActive ? 'Showing selected range' : 'Showing selected year';
