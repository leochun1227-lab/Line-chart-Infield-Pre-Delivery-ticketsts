...
def choose_best_match_per_ticket(so_item_df: pd.DataFrame) -> pd.DataFrame:
    """
    同一个 ticket 可能 raw 和 00+raw 都查到。
    优先保留 LookupSalesOrder == ERPFreeOrder 的结果；
    如果原值查不到，再保留补00的结果。
    """
    if so_item_df.empty:
        return so_item_df
...
def main():
    if not USERNAME or not PASSWORD:
        raise SystemExit("请先设置 C4C_USERNAME / C4C_PASSWORD")
    if "YOUR_USER" in SAP_HANA_DSN or "YOUR_PASSWORD" in SAP_HANA_DSN:
        raise SystemExit("请先设置 SAP_HANA_DSN")
