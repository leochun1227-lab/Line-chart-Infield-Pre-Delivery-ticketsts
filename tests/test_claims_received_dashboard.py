...
class ClaimsReceivedDashboardTest(unittest.TestCase):
    ...
    def test_payload_counts_all_matching_tickets_by_year_and_latest_current_year_data_month(self) -> None:
        ...
        self.assertEqual(payload["defaultRange"], {"mode": "last12", "startMonth": "2024-06", "endMonth": "2025-05"})
        self.assertEqual(len(payload["series"]), 12)
        self.assertEqual(payload["series"][6]["preDelivery"], 1)
        self.assertEqual(payload["series"][9]["inField"], 1)
        self.assertEqual(payload["series"][11]["preDelivery"], 1)
